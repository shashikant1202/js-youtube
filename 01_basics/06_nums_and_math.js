...
const score = 400
console.log(score); //js can automatically detect the data types but we can also define it
...
console.log(balance.toString().length); //here we got the length of the convertd string from number

console.log(balance.toFixed(2));  // it gives the precision value  means VALUE IN DECIMAL POINTS 

const otherNumber = 123.8987

console.log(otherNumber.toPrecision(4)); // it also gives the precise value upto the given digits in the  .toPrecision from left(starting) side of the number

const hundreds = 1000000
console.log(hundreds.toLocaleString('en-IN')); // by default it is in the US standard but we can change it to our standard 'en-IN' 
//us standars 100,000,000  INDIAN standard 1,00,00,000 we make like this by toLocaleString

Number  // put it in the cosole of browaer insepct
